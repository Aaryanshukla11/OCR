...
class PaddleOCREngine:
    ...
    def __init__(self):
        print("Initializing Singleton PaddleOCR Engine...")
        import paddle
        from paddleocr import PaddleOCR
        
        self.paddle = paddle
        self.device = "GPU" if paddle.is_compiled_with_cuda() and paddle.device.get_device().startswith("gpu") else "CPU"
        print(f"PaddleOCR Engine active on device: {self.device}")
        
        t0 = time.time()
        # Initialize PaddleOCR model (English + general OCR)
        try:
        ...
        print(f"PaddleOCR model loaded successfully in {init_dur:.2f}s")

    def _parse_paddle_output(self, raw_output):
        regions = []
        if not raw_output:
            return regions
    ...
    def process_image_file(self, file_path_or_np):
        t0 = time.time()
        try:
            res = self.ocr.ocr(file_path_or_np)
        except Exception:
            res = self.ocr.predict(file_path_or_np)
        elapsed = time.time() - t0
        regions = self._parse_paddle_output(res)
    ...
    def process_pdf(self, pdf_path):
        ...
        for idx in range(pages_count):
            ...
            regions, _ = self.process_image_file(np_img)
